...
//This program performs a collapse while keeping the collapsed variables labeled as they were pre-collapse
...
syntax, geovarsecvarnormfile(string) [geovarmsavarnormfile(string)] msavarnormfile(string) ///
	...
	//(8) Run the elasticity regression
	use `tf_array', clear
	regress `geosecpop'_log logpop_x_* secdummy*, nocons vce(cluster `msavar')
