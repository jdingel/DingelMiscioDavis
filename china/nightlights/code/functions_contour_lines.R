...
# contour lines
  ...
  # otherwise, we are best to use partitions to spatially index polygons
    ...
    china_bbox  <- raster::extent(sdf_polygon)              # full bounding box
    x_width  <- (china_bbox[2]-china_bbox[1])/n_parts       # moving window

    # moving window for partitions
    for (i in 1:n_parts){
      assign(
        paste("box",i,sep="_"),
        as(raster::extent(max(china_bbox[2]-(n_parts-i+1)*x_width,china_bbox[1]),
                          min(china_bbox[2]-(n_parts-i)*x_width,china_bbox[2]),
                          china_bbox[3],
                          china_bbox[4]),"SpatialLines")
      )
    }
