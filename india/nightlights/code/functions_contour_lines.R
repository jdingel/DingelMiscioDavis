...
# contour lines
  ...
  # otherwise, we are best to use partitions to spatially index polygons
    ...
    india_bbox  <- raster::extent(sdf_polygon)              # full bounding box
    x_width  <- (india_bbox[2]-india_bbox[1])/n_parts       # moving window

    # moving window for partitions
    for (i in 1:n_parts){
      assign(
        paste("box",i,sep="_"),
        as(raster::extent(max(india_bbox[2]-(n_parts-i+1)*x_width,india_bbox[1]),
                          min(india_bbox[2]-(n_parts-i)*x_width,india_bbox[2]),
                          india_bbox[3],
                          india_bbox[4]),"SpatialLines")
      )
    }
    ...
    if (rgeos::gIsValid(sdf_polygon)!=TRUE) {
      sdf_poly_proj <- sp::spTransform(sdf_polygon,sp::CRS(proj4))
      sdf_poly_proj <- rgeos::gBuffer(sdf_poly_proj,byid=T,width=0)
      sdf_polygon   <- sp::spTransform(sdf_poly_proj,og_proj)
    }
