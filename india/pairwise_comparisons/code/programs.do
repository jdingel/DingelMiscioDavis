...
syntax, binlist(numlist) binassignmentsfile(string) msagroupstub(string) ///
	...
	if 	"`geovar'"~="`msavar'" {
		...
		merge m:1 `geovar' using "`geovarmsavarnormfile'", keep(3) keepusing(`msavar') nogen
	...
	if "`secvarrankvalue'"~="`secvar'" {
		merge m:1 `secvar' using "`skillintensitynormfile'", keep(1 3) keepusing(`secvarrankvalue') nogen
	}
	...
	foreach bin of numlist `binlist' {
		...
		merge m:1 `msavar' using "`binassignmentsfile'", keepusing(`msagroupstub'`bin') nogen
		...
		if "`secvarrankvalue'"~="`secvar'" {
			 merge m:1 `secvar' using "`skillintensitynormfile'", keep(2 3) keepusing(`secvarrankvalue') nogen
		}
