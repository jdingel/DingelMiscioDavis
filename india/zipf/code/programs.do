...
////////////////////////////////
//India zipfplot comparison
////////////////////////////////

cap program drop  msa_zipf_compare_table_India
program define    msa_zipf_compare_table_India
	syntax using/, minimumpopulation(real) msalist(string) tablefilename(string) population(string)
	...
	use `tf0', clear
	replace geounitvar = subinstr(geounitvar,"msa","Light intensity ",1) if substr(geounitvar,1,3)=="msa"
	replace geounitvar = subinstr(geounitvar,"_raw"," ",1)
	replace geounitvar = subinstr(geounitvar,"_deblurred"," (deblurred nightlights)",1)

	sort geounitvar
	format beta r2 %4.3f
	listtab geounitvar beta se_GI r2 observations using "`tablefilename'", replace ///
...
graph twoway (scatter logrank logpop) (lfitci logrank logpop), ///
									graphregion(color(white)) ytitle(Log (rank-0.5)) ///
									xtitle(Log population) legend(off) ylab(,nogrid) ///
