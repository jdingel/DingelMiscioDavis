...
syntax, geovarsecvarnormfile(string) [geovarmsavarnormfile(string)] msavarnormfile(string) ///
	...
	//example from Brazil: use "/Users/jdingel/Box Sync/Yarin-Dingel/CAC_Brazil/data/data_norm1/BR_MUNICIPIO_edupop_Census2010.dta"
	...
	if 	"`geovar'"~="`msavar'" {
		...
		//(3) Collapse to MSA level
		collapsewithlabels `geosecpop', by(`msavar' `secvar' `extrageosecvars') statistic(sum)
